<template>
  <section class="panel"><div class="panel-heading"><h2>投稿初筛策略</h2><span class="badge">仅所有者可配置</span></div><p class="muted">默认人工审核。启用自动通过会公开符合策略的投稿，请核对条件后保存。所有检查只处理主动发布的公开快照。</p>
    ...
    <form v-else class="moderation-form" @submit.prevent="save"><fieldset :disabled="busy">
      ...
      <div class="moderation-numbers"><label>每作者 24 小时投稿上限<input v-model.number="draft.daily_limit" type="number" min="1" max="1000" required></label><label>自动通过：风险分数低于<input v-model.number="draft.approve_below" type="number" min="1" :max="draft.manual_at-1" required></label><label>高风险：转人工分数<input v-model.number="draft.manual_at" type="number" :min="draft.approve_below+1" max="100" required></label></div>
