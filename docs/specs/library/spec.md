...
本地提示词网格和行视图 MUST 展示已保存的第一张图片缩略图，按可见范围读取、支持离线与点击大图；不得为列表批量读取全部附件，具体行为见[本地缩略图计划](../../plans/2026-09-12-local-thumbnails.md)。
...
批量移动分类 MUST 只更新分类与修改时间，不回写旧正文、模型或附件；目标分类或提示词已删除时拒绝。
...
### Requirement: 本地回收站与撤销

删除提示词/合集后提供短时「撤销删除」；设置的数据页提供按标题查找的本地回收站，每次最多读取 200 条，重启后仍可恢复。恢复清除墓碑并生成新的修改时间，保留正文及库内附件，不覆盖已恢复记录；失效分类或合集归属改为未分类/独立提示词。合集删除仍释放成员，恢复合集不移动成员的当前归属。

- GIVEN 已删除的带文件提示词 WHEN 撤销或从回收站恢复 THEN 正文和文件完整可用，不生成重复记录。
- GIVEN 合集已删除且成员继续被整理 WHEN 恢复合集 THEN 恢复空合集，成员位置保持不变。

验证：原生 `recovery::tests`、`LocalTrash.spec.js`；见[使用体验计划](../../plans/2026-09-19-usability-safety.md)。
