...
用应用内完整页面管理本机偏好，呈现规则见 [ADR 0019](../../architecture/decisions/0019-settings-page.md)。设置不是弹窗，也不另开原生窗口。

M2 已落地且 MUST 保留：常规入口、启动器全局快捷键、JSON 导入预览与导出、库文件备份与恢复、浅色/深色主题。M8 只增加，不删除这些行为。

## Requirements

本机 AI 优化与翻译统一使用[翻译规格](../translation/spec.md)的接口配置及可搜索模型列表，沿用旧启动器凭据，不新增服务器密钥到客户端。

文件导入/恢复及独立我的发布入口 MUST 遵循[工作台优化计划](../../plans/2026-09-12-workbench-usability.md)切片 4。设置保留跳转入口，列表只在工作台展示；离开设置继续遵循未保存确认。选择/拖入 JSON 文件只预览，明确确认才写入；恢复选择器取消保留原路径，恢复前仍需二次确认。

### Requirement: 智能体 MCP 接入配置

网络与代理页 MUST 提供独立 MCP 程序路径校验和可复制的绝对路径配置，默认显式关闭广场工具；只能由用户勾选在所生成配置中启用，不继承桌面会话、不自动改宿主设置。浏览器无法核实原生路径时 MUST 提示使用桌面端。参数变化清除旧配置、复制失败保留配置供手动复制；验收见 [P2 计划](../../plans/2026-09-08-mcp-square.md)。
...
- THEN 页面适配主窗口，右侧表单居中限宽，导航和内容分别内部滚动
- AND 左上「返回应用」保留，系统窗口按钮和拖动区可用；返回后工作台查询、分类、视图和滚动不丢失

#### Scenario: 搜索设置分类

- GIVEN 左侧搜索框可用
- WHEN 输入分类名称或备份、代理等功能关键词
- THEN 导航只显示匹配分类，不自动跳页或清空未保存内容；零结果有提示，清空恢复十类

### Requirement: 统一设置布局

左侧 MUST 包含返回应用、搜索和设置导航，右侧显示当前页标题，不提供弹窗关闭叉。普通选项左说明右控件；多字段资料、账单与多行编辑 MUST 纵向分组，不挤在说明旁边。文本控件、选择框和开关有一致的尺寸、禁用与键盘焦点状态。
...
- THEN 导航位置不变，简单选项对齐，多字段内容独占可用宽度
- AND 所有原有设置入口、保存与错误提示保留，短视口只滚动内容，不裁掉返回入口
...
- GIVEN 用户打开外观页
- WHEN 使用鼠标或键盘选择系统、浅色或深色预览卡
- THEN 调用已有主题保存接口，选中状态反映生效值而非仅装饰
- AND 保存失败保留原主题并显示错误；禁用期间不重复提交
...
账号页 MUST 以已保存显示名、邮箱、文字头像与登录状态构成概览；不虚构第三方头像或登录方式。公开资料为主要编辑区，投稿与订阅单独分组，窄窗口顺序堆叠。资料修改显示未保存状态，保存中禁止重复提交，失败保留输入，成功后才更新概览并显示就近反馈。Mock 标识与真实权益始终可见，模拟操作和测试兑换放入默认折叠区域，展开后保留既有行为。

#### Scenario: 账号资料保存与测试区域层级

- GIVEN 已登录账号打开账号页
- WHEN 修改资料并保存，或展开模拟测试区域
- THEN 未保存/保存中/成功/失败有可见反馈，失败保留输入和旧概览，成功更新概览
- AND 默认折叠不遮住真实订阅状态或 Mock 标识，展开后模拟与测试兑换仍可用
- AND 访客仍有登录入口，窄屏卡片不横向溢出
...
同步页在既有偏好外 MUST 增加默认关闭的「本次包含私有附件」，显示目标账号及不公开、合并不删除的边界；不保存该授权，每次结束或账号变化后清除。同步过程中 MUST 禁止重复提交并显示当前传输步骤，失败给可重试原因；Wi-Fi 延后和保留本地跳过 MUST 明确说明，不冒充所有文件完成。具体数据行为见[同步规格](../sync/spec.md)。

## 测试映射

### 2026-09-19 状态反馈

本地库备份包含 SQLite 内保存的图片/附件及内嵌封面，外链只保存引用；ZIP 恢复需先解压选择数据库。恢复前必须在临时副本校验结构、附件及引用，显示数量并确认；确认时验证原备份（含 WAL）未变化，替换前保存当前库到独立备份，备份失败不得继续覆盖。

- GIVEN 带文件备份 WHEN 预览并确认恢复 THEN 显示库内文件数量、完整恢复文件并返回恢复前备份路径。
- GIVEN 备份损坏或预览后被修改 WHEN 恢复 THEN 拒绝替换，当前库保持可用。

启动时快捷键注册失败 MUST 在工作台底栏和快捷键设置显示修复入口；底栏仍可点击打开独立启动器，成功重新注册后清除告警。离线队列开关命名为「离线操作排队」，说明手动同步或之后成功收藏/发布会重试发送，不承诺后台自动同步个人库。

- GIVEN 注册失败 WHEN 查看底栏 THEN 不显示快捷键为正常可用，并可前往设置修复。
- GIVEN 已登录且有离线任务 WHEN 查看同步状态 THEN 仅显示当前账号待发送数量与最近手动同步结果；退出或换账号不展示上个账号记录。

验证：`shortcut.test.js`、`SyncStatus.spec.js`；实施与完整回归见[使用体验计划](../../plans/2026-09-19-usability-safety.md)。
