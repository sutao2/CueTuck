...
### Requirement: 推荐轮换与分页快照

`/browse` 推荐 MUST 使用精选标记、累计下载热度、30 天新鲜度与种子探索综合排序；默认每小时种子，桌面和网页提供「换一批」。推荐首批建立只含 ID 的有界快照，响应 `recommendation` 供后续请求原样传回；相同筛选/语言及种子下下载数和新增条目不改变本轮排序。实时下架内容不得返回，分页位置依原始快照前进，即使某页全部下架也允许空 items 搭配有效 next_offset。本轮 total 表示建立快照时匹配数，分类数量仍为当前全广场聚合。

快照限两小时、最多 64 份和合计 50 万 ID，单轮最多十万候选；过期、回收、进程重启后续页 MUST 返回 409 并提示重新加载，不静默切换新一轮。该版本为单 API 实例内存快照，多副本部署须共享快照后才能开启。最新维持上架时间/id，热门维持累计下载/标题/id，其他排序不是推荐快照；不宣称近期热度或个性化推荐。权重和验收见[推荐排序计划](../../plans/2026-09-15-recommendation-ranking.md)。

### Requirement: 跨语言用途搜索

标题、摘要（含已就绪中文译文）和作者检索支持共享用途词表中的中英文别名；英文词表词允许一次插入/删除/替换或相邻交换，中文仅处理词表明确别名，不声称任意语义翻译或任意中文错字识别。推荐搜索优先原标题字面命中，然后沿用本轮推荐权重；最新、热门仍保持所选排序。SQL 参数绑定，通配符按字面处理，不引入外部模型调用。原生、本地预览与网页使用同一词表，匹配文本安全高亮。场景与验收见[剩余优化计划](../../plans/2026-09-15-completion.md)。

### Requirement: 推荐反馈与换批去重

推荐支持本机不感兴趣清单（最多 200 项，达到上限须先恢复），可一键恢复；不跨账号同步。换批排除最近已加载的 48 条，API 接受最多 256 个长度不超过 200 的 ID，排除集参与快照键并在续页保持一致。最新、热门和收藏不受本机屏蔽影响；排除后的推荐总数不能改变全局分类数量。候选用尽提供清除最近记录的重新浏览入口，仍保留不感兴趣清单。场景见[剩余优化计划](../../plans/2026-09-15-completion.md)。

### Requirement: 直接使用

桌面广场卡片 MUST 读取完整正文后使用，不复制分页摘要。详情及合集成员 MUST 使用当前显示的语言版本；中文优先只使用与原稿匹配的已就绪译文，否则沿用可见原文，不自动生成译文。无变量直接复制，有变量填写并预览；空正文不得改剪贴板，复制失败保留正文和参数供重试。读取失败可重试，关闭详情后旧响应不得启动使用。

直接使用 MUST NOT 下载附件、新增本地副本、记录远端 ID 为本地使用记录或增加下载计数。列表主入口为使用/选择提示词、下载、收藏；标题进入详情，更多菜单不重复详情入口。场景及测试见[直接使用计划](../../plans/2026-09-15-direct-prompt-use.md)。

### Requirement: 卡片操作布局

广场网格在容器宽度足够时 MUST 保持至少 280px 列宽；窄容器允许单列收缩。使用/选择提示词文字不得拆行，图标不得被压扁，底栏在极窄空间允许整颗按钮换行。列数变化 MUST 同步虚拟行计算，列表视图布局不受影响。验收见[卡片窄列修复](../../plans/2026-09-18-square-card-width.md)。

### Requirement: 详情公开署名

已批准稿件的详情 MUST 返回并展示发布账号当前公开显示名和简介，字段与来源作者分离。未设置昵称显示「未设置昵称」，不以登录邮箱替代；不得返回邮箱、角色或凭据。原生 IPC 必须保留公开 publisher 字段。开源导入条目无平台发布者时展示已有来源作者，不虚构发布账号。缺省字段兼容旧服务。

- GIVEN 社区稿件已公开 WHEN 读取详情 THEN 标题下显示发布者公开昵称和可选简介，来源署名与许可保持。
- GIVEN 作者修改公开资料 WHEN 再次打开详情 THEN 使用新资料；无昵称时使用明确缺省文案。
- GIVEN 稿件未批准或条目已下架 WHEN 请求详情 THEN 沿用既有可见性规则，不额外暴露发布者资料。
...
### Requirement: 参考图并发与进度

参考图 MUST 每条最多三路并发，原生同批复用连接池，下一批重新读取代理。乱序完成后 MUST 仍按原图顺序保存。卡片、详情及补图显示已完成张数与总张数；保存前不宣布下载成功。已校验的公开参考图使用最多 32 MiB、5 分钟的进程内缓存（浏览器按字符串内存估算），关闭应用释放；仅用于公开参考图，不缓存账号附件。失败后的重试复用成功图片，只请求未完成项；复用仍生成新附件 ID 并执行整组校验。失败 MUST 取消剩余请求且不再启动排队图片，保持完整校验后原子导入与原有大小/来源限制。场景与验收见[下载优化计划](../../plans/2026-09-13-reference-download-speed.md)。
...
广场下载/收藏 MUST 使用带可访问名称和悬浮文字的图标；补充菜单仅通过可见更多按钮打开，卡片/列表不提供自定义右键菜单；补充菜单仅提供详情、已下载参考图补全，不重复主操作。行视图 MUST 显示可用参考图缩略图。成功下载沿用服务端确认计数；收藏请求成功后按已确认关系变化原地更新卡片和详情，离线排队不增加公开数量，不重载广场列表。完整场景见[交互计划](../../plans/2026-09-13-interactions-launcher-ai.md)，右键入口移除见[精简计划](../../plans/2026-09-15-remove-context-menu.md)。
