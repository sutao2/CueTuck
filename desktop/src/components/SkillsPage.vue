  ...
  <main ref="page" class="content-area skills-page" data-testid="skills-page">
    ...
    <template v-else>
      ...
      <template v-else-if="mode === 'local'">
        <p class="skills-category-caption muted">{{ skillCategoryName(category) }} · {{ filtered.length }} 个匹配 · 根据名称、目录和来源自动分类</p>
        ...
        <section class="skills-list" :aria-busy="busy"><button v-for="skill in visibleLocal" :key="skill.key" class="skills-list-row" :disabled="busy" @click="openDetail(skill)"><span class="skills-list-icon"><AppIcon name="skills"/></span><span class="skills-row-main"><strong>{{ skill.name }}</strong><small class="skills-category-label">{{ skillCategoryName(skill.category) }}</small><span>{{ skill.description || '未声明描述，打开查看说明与文件' }}</span></span><span class="skills-row-locations">{{ [...new Set(skill.installations.map(i=>`${rootName(i.root_id)} · ${scopeName(snapshot.roots.find(r=>r.id===i.root_id)?.scope)}`))].join(' · ') }}<small>{{ skill.installations.length }} 处安装</small></span><span class="skills-badge">{{ skill.source ? 'GitHub' : skill.status }}</span><span aria-hidden="true">›</span></button></section>
...
import {classifySkill,skillCategoryName,skillCategoryCounts} from '../platform/skillCategories.js';
...
const props=defineProps({mode:{type:String,default:'local'},category:{type:String,default:''}}),emit=defineEmits(['busy','categories']);
...
})})).filter(s=>(!props.category||s.category===props.category)&&s.installations.length&&(!sourceFilter.value||(sourceFilter.value==='github'?s.source:sourceFilter.value==='cache'?s.installations.some(i=>i.root_id.includes('cache')):sourceFilter.value==='local'?s.installations.some(i=>i.installed_digest&&!i.source):s.installations.some(i=>!i.installed_digest)))&&`${s.name} ${s.description} ${s.source?.repo||''}`.toLowerCase().includes(query.value.trim().toLowerCase())));
const visibleLocal=computed(()=>filtered.value.slice(localPage.value*pageSize,(localPage.value+1)*pageSize));
const remoteFiltered=computed(()=>categorizedRemote.value.filter(e=>(!props.category||e.category===props.category)&&`${e.name} ${e.directory} ${e.description||''}`.toLowerCase().includes(remoteQuery.value.trim().toLowerCase())));
