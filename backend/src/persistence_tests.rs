...
async fn collection_members_survive_publication_review_and_restart() {
    let state = postgres_state().await.expect("local Postgres required");
    let publication = Publication { cover: None, asset_refs: vec![], id: "collection-test".into(), source_id: "local".into(), status: "pending".into(),
...
async fn review_is_atomic_and_concurrent_decisions_cannot_overwrite_each_other() {
    ...
    let publication = Publication { cover: None, asset_refs: vec![], id: "atomic-review".into(), source_id: "local".into(), status: "pending".into(),
