<template>
  <section class="batch-organize" aria-label="批量整理" :aria-busy="busy">
    <div class="batch-controls">
      <strong>已选 {{ prompts.length }} 条</strong>
    ...
    <p class="use-hint">{{ mode === 'collection' ? '一条提示词只能属于一个合集，已归属其他合集的条目会移入目标合集。' : '只移动所选提示词的分类，正文与附件保持不变。' }}</p>
...
const props = defineProps({ prompts: { type: Array, required: true }, groups: Array, collections: Array, complete: { type: Function, required: true } });
