<template><section class="panel ai-jobs"><h3>AI 审核任务</h3><p>投稿先保存再异步审核；执行中断可恢复，最多尝试三次。模型失败或检查不完整时保留待审核，可继续人工处理。</p><button :disabled="busy" @click="load">刷新任务</button><p v-if="error" role="alert">{{ error }}</p><p v-if="message" role="status">{{ message }}</p><p v-if="items && !items.length">暂无任务</p><article v-for="item in items" :key="item.id"><strong>{{ item.kind === 'skill' ? 'Skill' : '提示词 / 合集' }} · {{ item.id }}</strong> · {{ labels[item.status] || item.status }} · {{ item.attempts }}/3<p>{{ item.error }}</p><button v-if="item.status==='failed' && item.attempts<3" :disabled="busy" @click="retry(item)">立即重试</button></article><footer><button :disabled="busy||!offset" @click="offset-=25;load()">上一页</button><span>{{ total }} 条</span><button :disabled="busy||offset+25>=total" @click="offset+=25;load()">下一页</button></footer></section></template>
<script setup>
...
async function retry(item){if(busy.value)return;busy.value=true;emit('busy-change',true);error.value='';message.value='';let queued=false;try{const data=await retryAiJob(item.id,item.kind);if(!data.queued)throw Error('服务器未确认入队');queued=true;message.value='已重新排队，不代表审核通过。'}catch(e){error.value=e.message}finally{busy.value=false;emit('busy-change',false)}if(queued)await load();}
onMounted(load);
</script>
