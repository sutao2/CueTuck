...
同步将覆盖本机未删除提示词的标题/正文或应用远端删除时，MUST 在同一落地事务中保存本机旧标题、正文、模型、分类及库内附件快照。保留最近 50 次，仅存本机，不进入同步白名单；失败事务不留下虚假版本。相同版本重放、仅元数据更新和保留本地策略不重复存快照。设置同步页可预览旧正文并恢复为新的独立本地副本，不改写当前同步胜出记录。

- GIVEN 本机正文被较新远端覆盖 WHEN 查看同步前版本并恢复 THEN 新副本拥有旧正文和旧库内附件，原记录仍是远端正文。
- GIVEN 同步批次包含非法引用 WHEN 落地失败 THEN 正文及版本历史一并回滚；重复同步不重复记录历史。

验证：原生 `history::tests`、`LocalVersions.spec.js`；实施见[使用体验计划](../../plans/2026-09-19-usability-safety.md)。
