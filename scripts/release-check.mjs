...
export function validateRelease({ config, version, cargoVersion, frontendUpdates, nativeUpdates, env = {}, production = false }) {
  const errors = [];
  ...
  if (production) {
    ...
    if (!env.TAURI_SIGNING_PRIVATE_KEY) errors.push('Release requires the matching updater signing key (not generated automatically)');
  }
  return errors;
}
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const errors = validateRelease({ config, version: JSON.parse(read('desktop/package.json')).version, cargoVersion: read('desktop/src-tauri/Cargo.toml').match(/^version = "([^"]+)"/m)?.[1], frontendUpdates: read('desktop/src/platform/updates.js'), nativeUpdates: read('desktop/src-tauri/src/commands/updates.rs'), env: process.env, production });
  errors.forEach(error => console.error(error));
  process.exitCode = errors.length ? 1 : 0;
  if (!errors.length) console.log(`${production ? 'Release configuration' : 'Local build configuration'} checks passed. Signing-key match, notarization and hosted update artifacts still require release verification.`);
}
