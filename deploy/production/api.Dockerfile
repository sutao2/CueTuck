...
COPY shared/translation.rs shared/skill_bundle.rs shared/search.rs shared/search-aliases.json /build/shared/
...
COPY --from=build /build/backend/target/release/promptark-api /usr/local/bin/promptark-api
USER 10001:10001
WORKDIR /data
EXPOSE 8787
CMD ["promptark-api"]
