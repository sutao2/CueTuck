...
### Requirement: 明确搜索范围与首次使用

桌面全局搜索增加「本机 Skills」与「社区 Skills」范围，后者仅显式选择才联网且遵守广场访问开关。本机搜索读取已登记目录，结果打开只读详情，不能自动安装；旧请求结果不得覆盖新范围。搜索结果的返回入口保留原浏览页。

- GIVEN 本机 Skill 或社区 Skill WHEN 在对应范围搜索并选中 THEN 进入既有详情/安装流程，安装仍需选择目标和确认。
- GIVEN 已从全局搜索打开 Skill 详情且无进行中的写入或确认 WHEN 点击可用的单一返回入口或按 Escape THEN 回到搜索前页面并保留原筛选；输入法组字的 Escape 不离开页面。
- GIVEN 编辑器有未保存草稿 WHEN 点击底栏同步状态或快捷键修复入口 THEN 先执行既有离开保护；选择继续编辑时保留草稿，不打开设置。
- GIVEN 本地库为空 WHEN 选择「用示例试一遍」 THEN 打开可编辑、可试填的未保存草稿；只有保存才新增记录，取消不创建记录。

本地卡片主操作按变量显示「复制提示词」或「填写并复制」；广场保存提示明确为「保存到本地」，收藏提示标明广场收藏夹。Skills 的备份和目录维护收进「管理与维护」，保留全部原操作。验收见[使用体验计划](../../plans/2026-09-19-usability-safety.md)。

### Requirement: 使用入口一致

本地提示词卡片仅保留一个使用主入口；无变量立即复制，有变量进入填写与预览，失败保留重试，不记录失败使用。详情切换语言后使用 MUST 传递当前显示正文，不改写保存的原稿。更多操作中的复制记录命名为「创建本地副本」，与复制到剪贴板分离。广场和启动器的对应行为见[直接使用计划](../../plans/2026-09-15-direct-prompt-use.md)。
...
提示词和 Skills 主要浏览页使用共用加载、空白和失败呈现；错误使用 alert 并提供重试，加载使用 status，筛选无结果提供清除查询或调整分类说明。空白状态不与错误同时显示；不以加载失败冒充零条目。滚动续页失败保留已有结果。验收见[剩余优化计划](../../plans/2026-09-15-completion.md)。

### Requirement: 搜索匹配反馈

本地提示词、合集、全局搜索与启动器使用[广场搜索词表合同](../square/spec.md)，保持分类范围与本地优先。标题与摘要用文本节点高亮，不解释 HTML；搜索摘要优先显示实际返回的 excerpt，不能将有摘要内容显示为空。启动器保持字面标题相关性高于使用偏好。

### Requirement: 发布前预览

提示词/合集投稿 MUST 先展示只读预览（标题、正文、分类、模型、所选公开附件），确认后才上传并提交。预览使用冻结快照，返回保留选择；账号变化/关闭页使预览失效。预览不自动选择附件。Skill 发布沿用完整文件清单、内容预览和授权确认。场景见[剩余优化计划](../../plans/2026-09-15-completion.md)。
...
### Requirement: 广场阅读与窄窗布局

桌面提示词与 Skill 广场 MUST 使用一致的页头和搜索控件密度。提示词详情正文先于参考图，足够宽时双栏、窄时单栏，无图不保留空侧列；动作栏在详情滚动面内保持可见。小于 900px 默认收起侧栏、允许临时展开，窄屏操作不覆盖已保存宽屏偏好。完整场景与验收见[布局计划](../../plans/2026-09-15-browse-layout.md)。
...
Windows 主窗口使用与侧栏同色的一体式顶栏，不叠加系统标题条；左侧保留折叠、当前位置及搜索，右侧提供最小化、最大化/还原和关闭。按钮不属于拖动区域，设置、登录、搜索或确认界面不能禁用窗口控制；关闭走现有原生关闭事件及托盘偏好。最大化状态从原生窗口读取，并在窗口状态变化后更新。此行为只作用于 Windows 主窗口，实施与场景见 [Windows 顶栏计划](../../plans/2026-09-21-windows-titlebar.md)。
