...
本地结果 MUST 在截取结果上限前按标题完全匹配、前缀、包含、其他命中分层；忽略英文大小写与首尾空白。同层级使用本机近期使用、累计使用、收藏信号；精确匹配不被这些信号越级覆盖。并列按标题和 ID 保持稳定；重新查询时读取最新记录，不记录失败复制，不上传使用偏好。规则与 Given/When/Then 验收见[启动器排序计划](../../plans/2026-09-15-launcher-ranking.md)，测试映射为 `launcherRanking.test.js` 和 `LauncherRanking.spec.js`。
...
系统 MUST 支持方向键移动高亮，Enter 对含变量提示词打开填写，无变量直接复制；修饰键+Enter 直接复制当前条。复制失败保留预览供重试。

- Given 列表展开或重新排列时鼠标静止 When 结果或快捷操作移到鼠标下方 Then 不改变键盘高亮；实际移动鼠标才更新高亮，快捷操作不抢搜索输入焦点。
...
- GIVEN 高亮提示词没有变量
- WHEN 按 Enter 自动复制被剪贴板拒绝
- THEN 显示错误且保留正文预览，不记录使用、不更新最近粘贴文本、不关闭窗口
...
非空输入 MUST 仅提供创建提示词、AI 优化、广场搜索，支持鼠标和键盘。创建仅在显式保存时写库；优化保留原文、展示可编辑结果，由用户采用或保存。无配置提供本机 AI 设置入口；失败不能覆盖原文或伪造结果。广场查询仅显式切换后启用，250ms 合并并取消旧请求，遵循广场访问开关；选择提示词读取完整正文和就绪中文版本并直接填写/复制，合集打开工作台详情选择成员，填写页保留查看详情。远端使用不写本地记录或计下载；失败保留输入，不自动下载/投稿。新增场景见[直接使用计划](../../plans/2026-09-15-direct-prompt-use.md)，既有行为见[交互计划](../../plans/2026-09-13-interactions-launcher-ai.md)。
