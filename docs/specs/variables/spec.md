...
导入的 `{argument name="名称" default="默认值"}` MUST 在桌面主窗口与独立启动器识别为显式命名变量并预填默认值；同名共享、默认值回退及无效语法保留规则见[导入参数计划](../../plans/2026-09-12-argument-variables.md)。不得迁移或改写用户保存的正文。
...
主窗口参数跳转和编辑试填 MUST 遵循[工作台优化计划](../../plans/2026-09-12-workbench-usability.md)切片 2，保留逐步填写与默认值规则。
