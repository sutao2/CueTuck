...
export async function startBillingCheckout(mockOutcome) {
  ...
  if (isTauri()) {
    const result=await tauriInvoke("start_billing_checkout", { access_token: token, mock_outcome: mockOutcome, request_id:requestId });
    pendingCheckout=null;return result;
  }
...
export async function redeemBillingCode(code, {mock=false}={}) {
  ...
  const key=`${token}:${mock}:${trimmed}`;
  if(mock&&pendingRedeem?.key!==key)pendingRedeem={key,id:crypto.randomUUID()};
  const requestId=mock?pendingRedeem.id:undefined;
  if (isTauri()) {
    const result=await tauriInvoke("redeem_billing_code", { access_token: token, code: trimmed, mock_mode:mock, request_id:requestId });pendingRedeem=null;return result;
  }
  const response = await fetch(`${apiBase()}/v1/billing/${mock?'mock/':''}redeem`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ code: trimmed, ...(mock?{request_id:requestId}:{}) }),
  });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) {
