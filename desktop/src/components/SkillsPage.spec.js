...
it('clears the previous publish package when the replacement folder cannot be prepared',async()=>{
 ...
 w=mount(SkillsPage,{props:{mode:'square'},attachTo:document.body});await flushPromises();await click('发布到社区');await click('从文件夹选择');await w.get('.skill-authoring input[type=checkbox]').setValue(true);expect(button('提交审核').attributes('disabled')).toBeUndefined();
...
it('shows only discovery and installation actions for GitHub sources',async()=>{
 w=mount(SkillsPage,{props:{mode:'square'},attachTo:document.body});await flushPromises();expect(button('创建 Skill')).toBeTruthy();expect(button('发布到社区')).toBeTruthy();
 await click('GitHub 来源');expect(button('创建 Skill')).toBeUndefined();expect(button('发布到社区')).toBeUndefined();
 await w.get('.skills-list-row').trigger('click');await flushPromises();expect(button('安装到…')).toBeTruthy();expect(button('发布到社区')).toBeUndefined();
 await w.get('.skills-back').trigger('click');await flushPromises();await click('社区广场');expect(button('创建 Skill')).toBeTruthy();expect(button('发布到社区')).toBeTruthy();
});
