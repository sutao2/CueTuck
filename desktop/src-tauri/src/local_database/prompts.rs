    ...
    let mut statement = connection
        .prepare(
            "SELECT p.id, p.title, p.summary, p.content, p.category_id, p.collection_id, COALESCE(p.use_count, 0),
             ...
             WHERE p.deleted_at IS NULL AND (COALESCE(p.source, 'local') != 'collection' OR NOT EXISTS(SELECT 1 FROM collections own WHERE own.id=p.collection_id AND own.deleted_at IS NULL))
