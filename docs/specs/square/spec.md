...
批量导入 MUST 不向正文追加来源或许可尾注；来源、许可及筛选证据单独保存。万条级列表 MUST 分页渲染，每页最多 48 条，保留总数与全部数据的搜索能力，筛选变更回到第一页。获取、去重、安全评分与本机幂等导入场景见[批量筛选计划](../../plans/2026-09-08-prompt-corpus.md)。自动筛选不得标称逐条人工审核或保证生成效果。
