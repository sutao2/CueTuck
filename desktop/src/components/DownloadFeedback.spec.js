...
it('shows a visible toast, updates counts and restores downloaded state after remount', async () => {
  ...
  expect(w.get('[data-testid="download-square"]').text()).toBe('已下载');
  ...
  expect(w.get('[data-testid="square-detail-download"]').text()).toBe('已下载');
  expect(w.get('[data-testid="square-detail-download"]').element.disabled).toBe(true);
