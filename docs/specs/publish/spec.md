...
单条提示词可显式选择附件投稿，未选附件 MUST 不进入快照或公开读取。附件稿件 MUST 人工审核，不能以文本规则或模型代替文件检查。权限、不可变引用、预览与下架场景见 [P1c 计划](../../plans/2026-09-08-publication-assets.md)。合集附件不在本片范围，保持正文快照。

### Requirement: 发布初筛与限额

服务端 MUST 在已启用自动审核时执行配置的日限额和公开快照初筛，不能依赖前端。关闭自动审核仍进入人工队列。新投稿记录实际结果与版本，旧投稿不得伪造初筛历史。本地规则自动通过须明确区分 AI 审核；任何必需检查不可用时转人工。

要求 AI 时，先持久化 pending，再在数据库事务外执行匹配的启用 Skill；只发送主动投稿文本（最多 32 KB），整个链路最多 25 秒。单模型失败可尝试主备，双模型一致/三模型多数必须有足够独立结果；各 Skill 均建议通过且最高风险分低于策略阈值、无本地待人工原因才允许自动上架。图片引用仍需人工，文本模型不能冒充视觉审核。最终保存须复核策略/规则/模型与 Skill 版本，并让先到的人工决定优先。提供商失败或进程中断不得使已保存投稿丢失。
...
合集发布 MUST 传 `kind=collection` 及非空 `members` 数组，每项仅含标题、正文、系统分类、模型。提交时读取当前成员，MUST NOT 上传本机路径、私有 ID 或使用记录。合集无成员、成员标题/正文为空、成员分类非法 MUST 拒绝。省略 kind 的旧请求仍按 prompt 处理。审核 MUST 展示成员正文；通过后保持合集类型与成员数量。成员快照不随本地后续编辑改变。合集封面资源上传不属于本切片，下载使用无封面合集。
