...
参考图 MUST 每条最多三路并发，原生同批复用连接池，下一批重新读取代理。乱序完成后 MUST 仍按原图顺序保存。卡片、详情及补图显示已完成张数与总张数；保存前不宣布下载成功。失败 MUST 取消剩余请求且不再启动排队图片，保持完整校验后原子导入与原有大小/来源限制。场景与验收见[下载优化计划](../../plans/2026-09-13-reference-download-speed.md)。
