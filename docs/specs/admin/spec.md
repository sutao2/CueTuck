...
### Requirement: 客户端一致的管理工作空间

管理台 MUST 复用客户端中性视觉 tokens 和线性图标，保持独立浏览器应用。分组导航 MUST 使用已有权限矩阵，覆盖全部管理页面；侧栏与内容独立滚动，账号入口持续可达。配置页限宽，长表格只在自身容器横向滚动；登录为简洁页面，不新增模态业务操作。视觉变更不得绕过写入锁、草稿保护或危险操作确认。场景与验收见[管理端视觉计划](../../plans/2026-09-12-admin-client-style.md)。
...
管理端提供 QQ / Foxmail 连接参数快捷填充，只修改未保存的主机、端口和加密方式，不覆盖账号、密码或启停，不自动保存或发信。QQ 主机须提示使用邮箱授权码而非登录密码。验收见 [QQ 邮件配置计划](../../plans/2026-09-12-qq-mail.md)。
...
模型的 vision 字段默认 false，必须明确授权图片发送；图片与文本共同进入所选 Skill 的视觉模型路由。未授权、类型或限额不符合、完整性错误、模型失败均不能假报通过。带附件的最终公开仍须人工确认，具体传输限制见[附件规格](../media/spec.md)。管理页可显式附带固定合成图片做图文样本测试；测试记录包含 image_sample，纯文本测试不冒充视觉实测。百炼兼容接口显式关闭思考模式并使用 max_tokens，其他接口保留既有参数。
