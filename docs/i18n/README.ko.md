...
데스크톱 프롬프트 런처입니다. 전역 단축키로 CueTuck을 열고 템플릿을 검색한 뒤 변수를 채워 복사하세요. 사용 중인 AI 도구에 바로 붙여 넣을 수 있습니다. 로컬 라이브러리, 참고 이미지와 Skills 관리로 자주 쓰는 작업 방식을 가까이 두세요.

[공식 웹사이트](https://prompt.likh.cn/) · [미리 보기 버전 다운로드](https://github.com/sutao2/CueTuck/releases) · [문서](../../docs/INDEX.md) · [문제 신고](https://github.com/sutao2/CueTuck/issues)

![CueTuck 독립 런처: 프롬프트 검색과 빠른 작업](../../docs/assets/readme/launcher-search.png)

## 단축키 하나로, 하던 작업을 계속

기본 단축키는 **Control + Space**이며 설정에서 변경할 수 있습니다. 키워드를 입력하고 **↑ / ↓**로 선택하세요. 변수가 없으면 **Enter**로 복사하고, 변수가 있으면 입력과 미리보기를 거쳐 결과를 복사합니다. 입력한 내용으로 프롬프트 생성, 직접 설정한 모델을 통한 AI 최적화, 광장 검색도 가능합니다.
